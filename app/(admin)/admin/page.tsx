...
import { MOCK_MEMBERS, MOCK_PRIVILEGES } from "@/lib/mock-data";
...
export default function AdminPage() {
  ...
  const approve = (id: string) => setMembers((ms) =>
    ms.map((m) => m.id === id ? { ...m, status: "active" as const, approvedAt: new Date().toISOString() } : m)
  );
  const reject  = (id: string) => setMembers((ms) =>
    ms.map((m) => m.id === id ? { ...m, status: "rejected" as const } : m)
  );

  return (
    <div className="min-h-screen bg-cream-100">

      {/* ── Admin Header (Midnight Green) ── */}
      ...
      <div className="max-w-6xl mx-auto p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: "Pending Approval",  value: pending.length, icon: Clock,       color: "bg-amber-50 text-amber-700" },
            { label: "Privileges Active", value: MOCK_PRIVILEGES.filter(p => p.isActive).length, icon: Gift, color: "bg-gold/10 text-gold-dark" },
                  ...
                  {pending.map((m) => (
                    <div key={m.id}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-ink-muted text-xs mt-0.5">{m.email} · {m.residentStatus} · {m.projectName}</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-cream-300 bg-cream-100">
                    {["Name", "Email", "Status", "Project", "Joined"].map((h) => (
                      <th key={h} className="px-5 py-3.5 text-left text-ink-muted text-xs font-semibold tracking-wide">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-cream-200">
                  {members.map((m) => (
                    <tr key={m.id} className="hover:bg-cream-100/60 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-ink-light">{m.email}</td>
                      <td className="px-5 py-3.5">
                        <span className={`text-xs px-2.5 py-0.5 rounded-full border capitalize font-medium ${STATUS_STYLES[m.status] ?? ""}`}>
                          {m.status.replace("_", " ")}
                        </span>
                      </td>
                      <td className="px-5 py-3.5 text-ink-muted text-xs">{m.projectName.replace("The Title ", "")}</td>
        ...
        {tab === "privileges" && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <button className="btn-gold text-xs px-4 py-2">+ Add Privilege</button>
            </div>
            <div className="bg-white border border-cream-300 rounded-2xl overflow-hidden shadow-card">
              <table className="w-full text-sm">
                <thead>
